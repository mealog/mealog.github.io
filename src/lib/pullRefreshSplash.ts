...
const SPLASH_ID = "hh-ptr-splash";

function injectSplashSpinKeyframes(): void {
  if (typeof document === "undefined") return;
  if (document.getElementById("hh-ptr-spin-keyframes")) return;
  const s = document.createElement("style");
  s.id = "hh-ptr-spin-keyframes";
  s.textContent = "@keyframes hh-ptr-spin{to{transform:rotate(360deg)}}";
...
export function mountPullRefreshSplashNow(): void {
  ...
  ring.style.cssText =
    "width:42px;height:42px;border:3px solid rgba(52,211,153,.2);border-top-color:rgba(52,211,153,.92);border-radius:50%;animation:hh-ptr-spin .72s linear infinite;";
