...
/**
 * 설정·초대 등 Google 로그인 직전 — 인앱 브라우저면 외부로 열 안내
 */
  ...
  return (
    <div className="mb-3 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs leading-relaxed text-amber-100/95">
      ...
      <p className="mt-1 text-amber-100/90">
        카카오톡·라인 등 안쪽 브라우저에서는{" "}
        <span className="text-amber-50">Google 보안 정책으로 로그인이 차단</span>될 수 있어요.{" "}
        {kind === "kakao"
          ? "아래 버튼으로 크롬·사파리에서 이 페이지를 연 뒤 로그인해 주세요."
          : kind === "line"
            ? "주소 복사 후 크롬·사파리에 붙여 넣거나, 상단 메뉴에서 ‘다른 브라우저’로 여세요."
            : "크롬·사파리 등 기본 브라우저에서 같은 주소로 열어 주세요."}
      </p>
      <div className="mt-2 flex flex-wrap gap-2">
        {kind === "kakao" && (
          ...
          >
            <ExternalLink size={13} /> Chrome / Safari에서 열기
          </button>
        )}
        {(kind === "line" ||
          kind === "instagram" ||
