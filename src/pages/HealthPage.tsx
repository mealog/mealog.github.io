...
export default function HealthPage() {
  ...
  return (
    <div className="flex flex-col gap-4 px-4 pt-5">
      ...
      {profile && <IdentityCard user={profile} authUser={authUser} />}
...
function IdentityCard({
  ...
  async function saveAll(next: User) {
    ...
    afterUserDataMutation();
    if (authUser) {
      // 공개 프로필과 기존 share 문서 일괄 갱신. 네트워크 실패해도 로컬엔 저장됨.
      try {
        await Promise.all([
...
/**
 * 공용 아바타 버블 — photoURL 이 있으면 이미지, 없으면 이니셜 원.
 */
